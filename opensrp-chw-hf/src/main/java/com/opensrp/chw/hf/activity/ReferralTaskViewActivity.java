...
import com.opensrp.chw.hf.listener.ReferralsTaskViewClickListener;
...
public class ReferralTaskViewActivity extends SecuredActivity {
    ...
    private void getReferralDetails() {
        if (getPersonObjectClient() != null) {
            ...
            referralDate.setText(DD_MM_YYYY.format(task.getExecutionStartDate().toDate()));
  ...
  /*  private Event createReferralCloseEvent() {
        ...
        try {
            ...
            JSONObject metadata = FormUtils.getInstance(getApplicationContext())
                    .getFormJson(com.opensrp.chw.core.utils.Utils.metadata().familyRegister.formName)
            ...
            formTag.providerId = com.opensrp.chw.core.utils.Utils.context().allSharedPreferences().fetchRegisteredANM();
            ...
            event = createEvent(new JSONArray(), metadata, formTag, familyMember.getFamilyID(),
                    CoreConstants.EventType.UPDATE_FAMILY_RELATIONS, com.opensrp.chw.core.utils.Utils.metadata().familyRegister.tableName);
            tagSyncMetadata(com.opensrp.chw.core.utils.Utils.context().allSharedPreferences(), event);
