...
public class DeadClientsUpdateActivity extends CoreFamilyRegisterActivity {
    ...
    public void startAncDangerSignsOutcomeForm() {
        ...
        try {
            ...
            String official_id = getIntent().getStringExtra(OFFICIAL_ID);
            if (death_cert == null) {
                death_cert = "";
            }
            ...
            valueMap.put(OFFICIAL_ID, official_id);
    ...
    private void updateDeathCertificate(AllCommonsRepository childCommonsRepository, AllCommonsRepository familyCommonsRepository, String jsonString) {
        try {
            ...
            JSONObject official_id = fields.getJSONObject(14);
            ...
            if (clientType != null) {
                switch (clientType) {
                    case "child":
                        updateDeathCertificateDB(childCommonsRepository, CoreConstants.TABLE_NAME.CHILD, issueDate, hasCertificate, deathCertificationNumber, deathNotificationDone, officialId);
                        break;
                    case "still":
                        updateDeathCertificateDB(familyCommonsRepository, CoreConstants.TABLE_NAME.FAMILY_MEMBER, issueDate, hasCertificate, deathCertificationNumber, deathNotificationDone, officialId);
                        break;
                    case "outOfArea":
                        updateDeathCertificateDB(familyCommonsRepository, CrvsConstants.TABLE_OUT_OF_AREA_DEATH, issueDate, hasCertificate, deathCertificationNumber, deathNotificationDone, officialId);
                        break;
                    case "adult":
                        updateDeathCertificateDB(familyCommonsRepository, CrvsConstants.TABLE_FAMILY_MEMBER, issueDate, hasCertificate, deathCertificationNumber, deathNotificationDone, officialId);
                        break;
                    default:
                        break;
                }
    ...
    private void updateDeathCertificateDB(AllCommonsRepository repository, String tableName, String issueDate, String hasCertificate, String deathCertificationNumber, String deathNotificationDone, String officialId) {
        String sql;
        String[] selectionArgs;
        if (!issueDate.equals("")) {
            sql = "UPDATE " + tableName + " SET received_death_certificate = ?, death_certificate_issue_date = ?, death_certificate_number = ?, official_id = ? WHERE id = ?";
            selectionArgs = new String[]{hasCertificate, issueDate, deathCertificationNumber, officialId, Objects.requireNonNull(getIntent().getStringExtra(BASE_ENTITY_ID)).toLowerCase()};
        } else {
            sql = "UPDATE " + tableName + " SET received_death_certificate = ?, death_notification_done = ?, official_id = ? WHERE id = ?";
            selectionArgs = new String[]{hasCertificate, deathNotificationDone, officialId, Objects.requireNonNull(getIntent().getStringExtra(BASE_ENTITY_ID)).toLowerCase()};
        }
        repository.customQuery(sql, selectionArgs, tableName);
    }
}