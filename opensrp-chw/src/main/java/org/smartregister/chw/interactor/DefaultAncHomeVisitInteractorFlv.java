...
public abstract class DefaultAncHomeVisitInteractorFlv implements AncHomeVisitInteractor.Flavor {
    ...
    private void evaluateTTImmunization(BaseAncHomeVisitContract.View view, LinkedHashMap<String, BaseAncHomeVisitAction> actionList, final MemberObject memberObject, VaccineTaskModel vaccineTaskModel, final Context context) throws Exception {
        ...
        String title = MessageFormat.format(context.getString(R.string.anc_home_visit_tt_immunization), details.getRight());
        ...
        TTAction helper = new TTAction(details, context);
        JSONObject jsonObject = getJson(Constants.JSON_FORM.ANC_HOME_VISIT.getTtImmunization(), memberObject.getBaseEntityId());
        JSONObject preProcessObject = helper.preProcess(jsonObject, details.getRight());

        BaseAncHomeVisitAction tt_immunization = new BaseAncHomeVisitAction.Builder(context, title)
                .withHelper(helper)
                .withOptional(false)
                .withDestinationFragment(BaseAncHomeVisitFragment.getInstance(view, null, preProcessObject, details.getRight()))
                .withVaccineWrapper(getVaccineWrapper(details.getMiddle(), vaccineTaskModel))
                ...
                .withSubtitle(MessageFormat.format("{0} {1}", dueState, DateTimeFormat.forPattern("dd MMM yyyy").print(new DateTime(details.getLeft()))))
                .build();
