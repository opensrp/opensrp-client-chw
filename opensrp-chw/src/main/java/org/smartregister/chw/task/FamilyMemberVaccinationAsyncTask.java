    ...
    protected Object doInBackground(Object[] objects) {
        ...
        if (cursor != null && cursor.moveToFirst()) {
            do {
                ...
                CommonPersonObject personObject = Utils.context().commonrepository(org.smartregister.chw.util.Constants.TABLE_NAME.CHILD).findByBaseEntityId(cursor.getString(1));
                ...
                List<Alert> alerts = ChwApplication.getInstance().getContext().alertService().findByEntityIdAndAlertNames(pClient.getCaseId(), VaccinateActionUtils.allAlertNames("child"));
                List<Vaccine> vaccines = ChwApplication.getInstance().vaccineRepository().findByEntityId(pClient.getCaseId());
